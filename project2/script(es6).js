console.log("Welcome to SA Library")
showBooks()
...
function showBooks() {
  ...
  <tr>
      ...
      <td>${element.type} </td>
      <td><button id= ${index} class="btn btn-primary mx-99" onclick="deleteBook(this.id)" style = "margin: auto;
      margin-right: 0px;
      float: right;">Delete Book</button></td>
  </tr>
      `

      let tableBody = document.getElementById('tableBody');
      if (bookObj.length ==0) {
        tableBody.innerHTML = `<strong>No Books to display</strong>`
      } else {
...
function deleteBook(index) {
  let getBooks = localStorage.getItem('books');
    ...
    bookObj.splice(index, 1)//kisi ek indx se sirf 1 element delete kar do 
    ...
    <div>
    <strong>Deleted</strong> Your Book has successfully deleted.</div>
  </div>
          `
...
class Display {
  add(book) {
    ...
    let tableBody = document.getElementById('tableBody')//when we are using es6 classes - we must use let
    let getBooks = localStorage.getItem('books');
    let bookObj;
    if (getBooks == null) {
      bookObj = [];
